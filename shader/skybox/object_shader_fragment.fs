...
void main()
{
   ...
   vec3 I = normalize(arg_world_pos - camera_pos);
   // vec3 R = refract(I, normalize(arg_world_normal), 0.85);
   vec3 R = reflect(I, -normalize(arg_world_normal));
   vec3 env = texture(tex_skybox, R).rgb; 

   // FragColor = vec4((animate + diffusion + specular + circle_v + env), 1.0);
